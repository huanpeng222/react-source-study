...
scrollTop = 700（用户滚到了最底部）
        ★ 因为 1000 - 300 = 700，正好"内容底部对齐可视区底部"
```

用户在底部正在看新消息。

**这时新消息追加**，scrollHeight 变成 1100（多了 100px 内容）。

**问题来了**：浏览器**不会自动调整 scrollTop**！它仍然是 700。
...
变更后：
  ...
  用户看到的区域：从 700px 到 1000px
  但内容已经到 1100px 了
  → 新消息（在 1000px ~ 1100px）用户看不到 ❌
  → 距离底部 = 1100 - 300 - 700 = 100px（被挤上去了）
```
...
class Chat extends Component {
  getSnapshotBeforeUpdate(prevProps, prevState) {
    ...
    //     1000           - 700      = 300（距底部 300px... 等等，应该是 0 对吧？
    //     精确说是 "可视区底部到内容底部的距离 + clientHeight" 之和）
  }
  ...
  componentDidUpdate(prevProps, prevState, snapshot) {
    ...
    list.scrollTop = list.scrollHeight - snapshot;
    //               1100           - 300      = 800
    //               变更后 scrollTop = 800，仍然在"距底部 300px"的位置
    //               1100 - 800 = 300 ✅ 距离底部不变
  }
}
```
...
| 时刻 | scrollHeight | scrollTop | 距离底部 |
|---|---|---|---|
| 变更前 | 1000 | 700 | 1000 - 700 = 300 |
| 快照记下 | — | — | **snapshot = 300** |
| 变更（新消息追加） | 1100 | 700（浏览器没动）| 1100 - 700 = 400 ⚠️ 用户被挤上去 100px |
| componentDidUpdate 修正 | 1100 | 1100 - 300 = **800** | 1100 - 800 = 300 ✅ 恢复 |

**核心思想**：

> 不用记"绝对位置"，记**"相对底部距离"**。变更前后保持相对距离不变 = 用户感觉滚动位置没动。
...
|---|---|
| 正在看最新消息 | 滚动条贴底 |
| 收到一条新消息 | **画面"跳"了一下，被挤上去 100px** 😡 |
| 想看新消息 | 必须手动往下滚 |

→ 微信、Slack、Discord 用着不卡顿 = 它们都用了类似 getSnapshotBeforeUpdate 的机制。
