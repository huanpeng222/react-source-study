...
## 六点五、跟练追问记录（2026-07-07）

> **追问原文**："startTransition 的调用不会阻塞主线程是因为它的执行是在主线程空闲后执行的吗？降低优先级主要是把 startTransition 的回调函数延迟执行吗？用 useDeferredValue 包裹的数据 query，是指拿到最新的 query 时，不着急更新页面数据吗？为啥需要 useDeferredValue，不太理解"

**核心误解**：把"低优先级"理解成了"延后执行这段代码本身"。需要先掰开两层概念：

```
"喊单"（setState 调用 / callback 执行）—— 永远是立刻/同步的，从不被推迟
"做菜上桌"（React 真正的渲染计算）—— 这才是被安排、被打断的部分
```

`startTransition(callback)` 的 `callback()` 是**同步立刻执行**的，不是"等主线程空闲才执行"——React 也根本不依赖"主线程空闲"这种被动等待，Day19 讲过 React 自己实现 Scheduler、用 MessageChannel 控制时间片，不用原生 `requestIdleCallback`。被降低优先级的是 callback 里 setState 引发的**后续渲染计算**，不是 callback 这段代码。

**useDeferredValue 为什么需要**：因为同一个 `query` 值往往身兼两职——驱动 input 立刻显示 + 驱动 HeavyList 做慢计算。如果不处理，两者绑在同一次渲染里，慢的拖累快的（打字卡顿）。`useDeferredValue` 给你一份"慢半拍镜像"（`deferredQuery`），只喂给慢组件，query 本身该多快还是多快。

**为什么不能直接把 setQuery 包进 startTransition**：因为那样 query 本身也会变慢，input 显示新字符也会跟着延迟——这是因为 query 同时承担两个职责，没法只把其中一个职责标记成"不急"。这正是"能改代码用 useTransition；只能改读到的值用 useDeferredValue"这句话背后的具体原因：当一个值身兼多职时，只能在"消费端"给慢组件单独一份延迟镜像，没法在"产生端"整体降优先级。
...
三个实验改为在真实 Vite + React 项目里跑（浏览器环境，能观察 isPending/DevTools Profiler，比 jsdom 更贴近真实场景），详见 `demos/day20/README.md`：

| 实验 | 验证什么 |
| -- | ------------------------------------------------------------------------------------ |
| T1 | `startTransition` 包裹的 setState 是否真的引发 isPending 的 true→false 变化序列 |
| T2 | 高优先级更新和 transition 更新同时发起时，最终结果以哪个为准 |
