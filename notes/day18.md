...
四个实验全部在真实 Next.js 项目里跑（`npx create-next-app` + App Router），详见 `demos/day18/README.md`：

| 实验 | 验证什么 |
|---|---|
| P1 | Parallel Routes 的插槽独立渲染、独立 Suspense 边界（一个插槽慢加载不阻塞另一个） |
| P2 | Intercepting Routes 的"软导航显示模态框 vs 硬导航显示完整页面" |
| P3 | Middleware matcher 是否生效 + redirect（URL变）/rewrite（URL不变）实际效果 |
| P4 | 故意把 `server-only` 模块 import 进 `'use client'` 组件，验证构建报错 |
